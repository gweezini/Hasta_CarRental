...
<div class="max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm flex flex-col h-full border-t-4 border-[#cb5c55]">
            ...
            <div class="w-full bg-gray-100 rounded-lg mb-4 border border-gray-300 relative group cursor-pointer" 
                ...
                @if($booking->payment_receipt)
                    ...
                    <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-10 transition duration-300 flex items-center justify-center">
                        <span class="bg-black bg-opacity-70 text-white text-xs px-3 py-1 rounded opacity-0 group-hover:opacity-100 transition">Click to Zoom</span>
                    </div>
                @else
            ...
            @if($booking->payment_receipt)
                <div class="text-center mb-6">
                    <p class="text-sm text-gray-500 uppercase font-bold">Total Rental Fee</p>
                    <p class="text-3xl font-bold theme-text">RM {{ number_format($booking->total_rental_fee, 2) }}</p>
                </div>

                <div class="grid grid-cols-2 gap-3 mt-auto sticky bottom-0 bg-white pt-2">
                    <form action="{{ route('admin.payment.reject', $booking->id) }}" method="POST" onsubmit="return confirm('Reject this booking?');">
                        @csrf
                        <button class="w-full py-3 bg-white text-red-600 font-bold rounded-lg border border-red-200 hover:bg-red-50">Reject</button>
                    </form>

                    <form action="{{ route('admin.payment.approve', $booking->id) }}" method="POST" onsubmit="return confirm('Approve this booking?');">
                        @csrf
                        <button class="w-full py-3 bg-[#cb5c55] text-white font-bold rounded-lg hover:opacity-90">Approve</button>
                    </form>
                </div>
            @endif
        </div>
