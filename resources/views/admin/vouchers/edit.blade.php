...
<div class="max-w-4xl mx-auto pb-12">
    <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        ...
        <div class="px-10 py-8 border-b border-gray-50 bg-gray-50/30">
            <h2 class="text-2xl font-black text-gray-800 tracking-tight">Modify Voucher</h2>
            <p class="text-gray-400 text-sm font-medium mt-1">Updating settings for voucher: <span class="bg-[#cb5c55]/10 text-[#cb5c55] px-2 py-0.5 rounded-lg ml-1">{{ $voucher->code }}</span></p>
        </div>

        <form action="{{ route('admin.vouchers.update', $voucher->id) }}" method="POST" class="p-10 text-left">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                ...
                <div class="col-span-2 md:col-span-1 space-y-2">
                    ...
                    <div class="relative">
                        <select name="type" id="type" class="w-full rounded-2xl border-2 border-gray-100 bg-gray-50/50 focus:bg-white focus:border-[#cb5c55] focus:ring-0 py-3.5 px-5 font-black text-gray-800 appearance-none cursor-pointer transition-all">
                            <option value="fixed" {{ old('type', $voucher->type) == 'fixed' ? 'selected' : '' }}>Fixed RM Discount</option>
                            <option value="percent" {{ old('type', $voucher->type) == 'percent' ? 'selected' : '' }}>Percentage (%) Off</option>
                            <option value="free_hours" {{ old('type', $voucher->type) == 'free_hours' ? 'selected' : '' }}>Free Hours Reward</option>
                        </select>
                ...
                {{-- Expiry Date --}}
                <div class="col-span-2 space-y-2" x-data="{ hasExpiry: {{ $voucher->expires_at ? 'true' : 'false' }} }">
